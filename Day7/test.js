...
function startBalapan() {
    return new Promise((resolve, reject) => {
        let pemenang = null;
        let jarakPemenang = 0;

        process.stdin.setRawMode(true); // Mengatur mode raw untuk menerima input per karakter
        process.stdin.resume(); // Melanjutkan pembacaan input

        process.stdin.on('data', (key) => { // Menambahkan event listener untuk menerima input
            mobils.forEach(mobil => {
                mobil.maju(); // Memperbaharui posisi mobil
                if (mobil.posisi >= panjangTrack && (pemenang === null || mobil.posisi > jarakPemenang)) {
                    pemenang = mobil;
                    jarakPemenang = mobil.posisi;
                }
            });
            console.log('\n', '\n', '\n', '\n', '\n', '\n', '\n', '\n', '\n', '\n', '\n', '\n', '\n', '\n', '\n'); // Tambahkan baris kosong untuk memisahkan setiap iterasi
            mobils.forEach(mobil => console.log(mobil.render())); // Menampilkan visualisasi track


            if (pemenang !== null) { // Jika ada pemenang, menampilkan nama pemenang dan menyelesaikan promise
                console.log(`Pemenang balapan: ${pemenang.nama}`);
                process.stdin.setRawMode(false); // Mengembalikan mode stdin ke mode normal
                process.stdin.pause(); // Menjeda pembacaan input
                resolve();
            }
        });
    });
